...
from ImageFolder_vindr import (
    ImageFolder_vindr,
    ChestX_ray14,
    ShenzhenCXR,
    LDPolyp,
    CombinedImageFolderDataset,
)
...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        default=True,
        type=utils.bool_flag,
        help="Whether to use global loss",
    )
    parser.add_argument(
        "--matrix_matching",
        default=False,
        type=utils.bool_flag,
        help="Whether to use matrix matching loss",
    )
    parser.add_argument(
        "--comp",
        default=False,
        type=utils.bool_flag,
        help="Whether to composition and decomposition loss",
    )
    ...
    parser.add_argument(
        "--epochs", default=302, type=int, help="Number of epochs of training."
    )
    parser.add_argument(
        "--freeze_last_layer",
        default=1,
    ...
    parser.add_argument(
        "--data_path",
        default="/data1/zhouziyu/liang/NIHChestXray/images/images_all/",
        type=str,
        help="Please specify path to the ImageNet training data.",
    )
    parser.add_argument(
        "--output_dir",
        default="/ssd2/zhouziyu/ssl/github/ACE/pretrained_weight/from_imagenet_global_triplet",
        type=str,
    ...
    parser.add_argument(
        "--cfg",
        default="./swin_configs/swin_base_img224_window7.yaml",
        type=str,
        metavar="FILE",
...
def train_dino(args):
    ...
    #     args.data_path, "./data/xray14/official/train_val.txt", augment=transform
    # )
    dataset = CombinedImageFolderDataset(
        root_dir="/data/jliang12/ssiingh/datasets/SSL_root", augment=transform
    )
    ...
    utils.init_from_imagenet(
        "/ssd2/zhouziyu/ssl/contrast_12N/pretrained_weight/ldpolyp/swin_base_patch4_window7_224_imagenet1k.pth",
        student,
        teacher,
    )
...
def train_one_epoch(
    ...
    for it, (
        ...
        _,
    ) in enumerate(metric_logger.log_every(data_loader, 50, header, log_writer)):
        ...
        sample_index = torch.cat(
            [
                sample_index1.cuda(non_blocking=True),
                sample_index2.cuda(non_blocking=True),
            ]
        )
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            teacher_cls, teacher_spatial, teacher_comp, teacher_decomp = teacher(
                images, local_crops, sample_index
            )
            student_cls, student_spatial_pred, student_comp, student_decomp = student(
                images, local_crops, sample_index
            )  # global embedding of student, local embeddings of teacher, local embeddings of student
            ...
            if args.matrix_matching:
                loss1, loss2 = barlow_loss(
                    teacher_spatials,
                    student_spatials_pred,
                    locations[0],
                    locations[1],
                    s2lmapping.cuda(),
                    l2smapping.cuda(),
            ...
            if args.comp:
                ...
                # # ipdb.set_trace()
                for i in range(4):
                    loss_decomp += F.smooth_l1_loss(
                        student_decomp[i], teacher_decomp[i]
                    )
            ...
            with torch.no_grad():
                ...
                for (name_q, param_q), (name_k, param_k) in zip(
                    student.module.named_parameters(),
                    teacher_without_ddp.named_parameters(),
                ):
...
class NonLinearConv(nn.Module):
    def __init__(self, in_channels, hidden_channels):
        ...
        self.expand_conv = nn.Conv2d(
            in_channels=1, out_channels=hidden_channels, kernel_size=1
        )
...
class AttentionMLPModel(nn.Module):  # compute matrix matching loss
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        # Attention Layer
        self.attention = nn.ModuleDict({"attention_layer": AttentionLayer()})
        # self.loss_view1 = sigmoid_focal_loss(alpha=0.75)#nn.BCEWithLogitsLoss()
        self.loss_view2 = nn.CrossEntropyLoss(
    ...
    def forward(
        ...
        loss1 = (
            sigmoid_focal_loss(logits_A, l2smapping.cuda(), alpha=0.9, gamma=0).mean()
            + sigmoid_focal_loss(
                logits_A_, l2smapping.cuda(), alpha=0.9, gamma=0
            ).mean()
        ) / 2
        loss2 = (
            sigmoid_focal_loss(logits_B, s2lmapping.cuda(), alpha=0.9, gamma=0).mean()
            + sigmoid_focal_loss(
                logits_B_, s2lmapping.cuda(), alpha=0.9, gamma=0
            ).mean()
        ) / 2
...
class TripletLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.triplet_loss = InfoNCE(
            temperature=0.2, negative_mode="unpaired"
        )  # nn.TripletMarginLoss(margin=2, p=2)
    ...
    def compute_loss(self, crop1, crop2, bce_labelsl2s, bce_labelss2l):
        ...
        norm_factor_expanded = norm_factor.squeeze(1).unsqueeze(
            -1
        )  # Change shape from (b, 196) to (b, 196, 1)
        # Calculating average feature
        ##print(norm_factor_expanded.shape)
        average_feature_2_match_1 = (
            torch.bmm(bce_labelsl2s, crop2) / norm_factor_expanded
        )  # [B,196,512]
        ...
        for i in range(crop1.shape[0]):
            if (
                len(negative_indices1[0]) == 0
                or len(negative_indices2[0]) == 0
                or crop1[i][crop1_index[i]].shape[0] == 0
            ):
                continue
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, teacher_cls, student_cls):
        ...
        loss1 = self.contrastive_loss(
            teacher_overlap_crop1,
            student_overlap_crop2,
            torch.cat(
                (
                    teacher_nonoverlap_crop1.unsqueeze(1),
                    student_nonoverlap_crop2.unsqueeze(1),
                ),
                dim=1,
            ),
        )  # negatives [B,2,1024]
        loss2 = self.contrastive_loss(
            teacher_overlap_crop2,
            student_overlap_crop1,
            torch.cat(
                (
                    teacher_nonoverlap_crop2.unsqueeze(1),
                    student_nonoverlap_crop1.unsqueeze(1),
                ),
                dim=1,
            ),
        )
...
class DINOLoss_Overlap(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        loss1 = torch.sum(
            -teacher_overlap_crop2 * F.log_softmax(student_overlap_crop1, dim=-1),
            dim=-1,
        ).mean()
        loss2 = torch.sum(
            -teacher_overlap_crop1 * F.log_softmax(student_overlap_crop2, dim=-1),
            dim=-1,
        ).mean()
...
class DINOLoss_Triplet(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        loss1 = (
            (
                torch.norm(teacher_overlap_crop1 - student_overlap_crop2, p=2, dim=1)
                + self.eps
            )
            / (
                torch.norm(
                    teacher_nonoverlap_crop1 - student_nonoverlap_crop2, p=2, dim=1
                )
                + self.eps
            )
        ).mean()
        loss2 = (
            (
                torch.norm(teacher_overlap_crop2 - student_overlap_crop1, p=2, dim=1)
                + self.eps
            )
            / (
                torch.norm(
                    teacher_nonoverlap_crop2 - student_nonoverlap_crop1, p=2, dim=1
                )
                + self.eps
            )
        ).mean()
