...
def get_args_parser():
    ...
    parser.add_argument(
        "--output_dir",
        default="/ssd2/zhouziyu/ssl/github/ACE/pretrained_weight/from_imagenet_global_12N",
        type=str,
...
def train_one_epoch(
    ...
    for it, ((images, locations, s2lmapping, l2smapping), _) in enumerate(
        metric_logger.log_every(data_loader, 50, header, log_writer)
    ):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            student_cls, student_spatial, student_spatial_pred = student(
                images
            ...
            loss_local = local_loss(
                student_spatials[0],
                student_spatials_pred[1],
                l2smapping.cuda(),
                s2lmapping.cuda(),
            ) + local_loss(
                student_spatials_pred[0],
                student_spatials[1],
                l2smapping.cuda(),
                s2lmapping.cuda(),
            ...
            loss = (
                loss_vic + loss_local + global_loss
            ) / 2  # loss_local#loss_local #loss_vic#(+loss_local)/2#(loss_dino + order_loss+ loss_vic+restor_loss)/4
            # loss = loss_local
...
class AttentionMLPModel(nn.Module):  # compute matrix matching loss
    ...
    def forward(
        ...
        loss1 = (
            sigmoid_focal_loss(logits_A, l2smapping.cuda(), alpha=0.99, gamma=0).mean()
            + sigmoid_focal_loss(
                logits_A_, l2smapping.cuda(), alpha=0.99, gamma=0
            ).mean()
        ) / 2
        loss2 = (
            sigmoid_focal_loss(logits_B, s2lmapping.cuda(), alpha=0.99, gamma=0).mean()
            + sigmoid_focal_loss(
                logits_B_, s2lmapping.cuda(), alpha=0.99, gamma=0
            ).mean()
        ) / 2
