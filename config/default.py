...
_C.TRAIN.IMAGE_SIZE = [512, 512]  # width * height, ex: 192 * 256
...
_C.TEST.IMAGE_SIZE = [512, 512]  # width * height, ex: 192 * 256
...
def get_lr_scheduler_name(config):
    ...
    if lr_scheduler.METHOD == "MultiStep":
        setting = "{}factor{}".format(
            "_".join([str(i) for i in lr_scheduler.MILESTONES]), lr_scheduler.GAMMA
        )
        ...
        setting = "{}".format(lr_scheduler.WARMUP_EPOCH)
    else:
        raise ValueError("Unknown lr scheduler: {}".format(lr_scheduler.METHOD))

    return template.format(
        method=lr_scheduler.METHOD, setting=setting, ep=config.TRAIN.END_EPOCH
    )
...
def get_train_name(config, args):
    ...
    if mixup > 0.0:
        ...
        aug = f"mixcut{mixcut}"
    template = (
        "labelsmooth{ls}{aug}{optim}-lr{lr:.5g}wd{wd}bs{bs}X{world_size}-{lr_scheduler}"
    )
...
def get_config_name(config):
    ...
    return template.format(
        model_name=model_name, input_size=input_size, aug=aug, train_cfg=train_cfg
    )
