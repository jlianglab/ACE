...
import cv2
from crop import img_transforms,get_index, get_corresponding_indices, get_patch_matching_target_1to1, get_corresponding_indices_comp
from crop import PatchCrop, PatchCropViT, PatchCropSwinV2
...
class RandomCenterCropResize:
    def __init__(self, target_size=448, scale=(0.4, 1.0), ratio=(3/4, 4/3), interpolation=Image.BICUBIC):
    ...
    def __call__(self, img):
        ...
        # Randomly select a crop area within the given scale
        crop_area = random.uniform(self.scale[0], self.scale[1]) * area

        # Randomly select an aspect ratio within the given range
        aspect_ratio = random.uniform(self.ratio[0], self.ratio[1])
        crop_w = int(round((crop_area * aspect_ratio) ** 0.5))
        crop_h = int(round((crop_area / aspect_ratio) ** 0.5))

        # Ensure crop dimensions do not exceed the original image size
        ...
        # Perform center cropping and resize to target size
        img = img.crop((left, top, right, bottom))
        img = img.resize((self.target_size, self.target_size), self.interpolation)
        return img
...
class DataAugmentationDINO(object):
    def __init__(self,args):
        ...
        if args.arch == 'swin_base':
            self.img_transforms = PatchCrop(size=1024)
            self.overlap_initial_crop=transforms.RandomResizedCrop(1024, scale=(0.85,1.0),interpolation=Image.BICUBIC)
        elif args.arch == 'vit_base': # the same setting with DINOv2, 518*518
            self.img_transforms = PatchCropViT(size=1260)
            self.overlap_initial_crop=transforms.RandomResizedCrop(1260, scale=(0.85,1.0),interpolation=Image.BICUBIC)
        elif args.arch == 'swinv2_base': 
            self.img_transforms = PatchCropSwinV2(size=1152)
            self.overlap_initial_crop=transforms.RandomResizedCrop(1152, scale=(0.85,1.0),interpolation=Image.BICUBIC)
        ...
        self.global_transform1 = transforms.Compose([
            # transforms.RandomResizedCrop(448, scale=args.global_crops_scale, interpolation=Image.BICUBIC),
            RandomCenterCropResize(target_size=args.image_size, scale=args.global_crops_scale),
            basic_trans,
            GaussianBlur(1.0),
            normalize,
        ])
        ...
        self.local_transform = transforms.Compose([
            RandomCenterCropResize(target_size=args.image_size, scale=args.local_crops_scale),
            # RandomCenterCropResize(196, scale=args.local_crops_scale),
            basic_trans,
            GaussianBlur(p=0.5),
            normalize,
        ])
    ...
    def __call__(self, image, base_crop_size, epoch):
        crops = []

        if epoch%3==0: 
            patch = transforms.Compose([transforms.RandomCrop(base_crop_size)])(image)
            crops.append(self.global_transform1(patch))
            crops.append(self.global_transform2(patch))
            for _ in range(self.local_crops_number):
                crops.append(self.local_transform(patch))


            return crops
        elif epoch%3==1: 
            # image = transforms.Compose([transforms.RandomCrop(base_crop_size),
                                        # transforms.Resize(1024)])(image)
            image = self.overlap_initial_crop(image)
            image = np.asarray(image)

            patch, (idx_x1, idx_y1), (idx_x2, idx_y2), (k, l) = self.img_transforms(image, k=1, l=1) # get the two crops, the top left corner indexed of two crops, the size rate of the bigger crop1
            if self.args.arch == 'swin_base':
                sample_index1, sample_index2 = get_index((idx_x1, idx_y1), (idx_x2, idx_y2), (k, l), patch_num=14) # the overlap mask of two crops (all 14*14)
                target_matrix = get_patch_matching_target_1to1(sample_index1, sample_index2, patch_num=14) # [196,196]
            elif self.args.arch == 'vit_base':
                sample_index1, sample_index2 = get_index((idx_x1, idx_y1), (idx_x2, idx_y2), (k, l), patch_num=37)
                target_matrix = get_patch_matching_target_1to1(sample_index1, sample_index2, patch_num=37)
            elif self.args.arch == 'swinv2_base': 
                sample_index1, sample_index2 = get_index((idx_x1, idx_y1), (idx_x2, idx_y2), (k, l), patch_num=16) # the overlap mask of two crops (all 14*14)
            ...
            # ipdb.set_trace()
            # print((idx_x1, idx_y1), (idx_x2, idx_y2), target_matrix)
            ...
            # print(torch.count_nonzero(target_matrix), (target_matrix == 1).sum().item())
            # np.savetxt('target_matrix2.txt', target_matrix.numpy(), fmt='%.4f')
            # np.savetxt('images2.txt', crops[0].copy().numpy(), fmt='%.4f')

            return crops, target_matrix#, (idx_x1, idx_y1), (idx_x2, idx_y2)
        else:
            ...
            patch, (idx_x1, idx_y1), (idx_x2, idx_y2), (k, l) = self.img_transforms(image, k=2, l=2) # get the two crops, the top left corner indexed of two crops, the size rate of the bigger crop1
            if self.args.arch == 'swin_base':
                sample_index1, sample_index2 = get_index((idx_x1, idx_y1), (idx_x2, idx_y2), (k, l), patch_num=14) # the overlap mask of two crops (all 14*14)
                s2lmapping,l2smapping = get_corresponding_indices_comp(sample_index1, sample_index2,(k, l), patch_num=14) # two target matrices of matrix matching, size [196,49], [196,784]
            elif self.args.arch == 'vit_base':
                sample_index1, sample_index2 = get_index((idx_x1, idx_y1), (idx_x2, idx_y2), (k, l), patch_num=37)
                s2lmapping,l2smapping = get_corresponding_indices_comp(sample_index1, sample_index2,(k, l), patch_num=37)
            elif self.args.arch == 'swinv2_base': 
                sample_index1, sample_index2 = get_index((idx_x1, idx_y1), (idx_x2, idx_y2), (k, l), patch_num=16)
                s2lmapping,l2smapping = get_corresponding_indices_comp(sample_index1, sample_index2,(k, l), patch_num=16)
            ...
            return crops, s2lmapping, l2smapping