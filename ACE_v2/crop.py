...
def get_index(a, b, c, patch_num=14): 
...
def upsample(input_bool, patch_num): # [196,]
    input = input_bool.reshape(patch_num,patch_num).float()
    output_float = F.interpolate(input.unsqueeze(0).unsqueeze(0), size=(patch_num*2, patch_num*2), mode='nearest')
    output_float = output_float.squeeze(0).squeeze(0).reshape(-1) # [784,]
    output_float = torch.nonzero(output_float)
    return output_float

def downsample(input_bool, patch_num): # [196,]
    input = input_bool.reshape(patch_num,patch_num).float()
...
def get_corresponding_indices_comp(overlap_mask_1, overlap_mask_2, kl, patch_num=14):
    """
    input: 
    ...
    # print(len(true_indices_2), len(true_indices_1))
    upsample_indices_1 = upsample(overlap_mask_1, patch_num) # overlap patches:4n
    downsample_indices_2 = downsample(overlap_mask_2, patch_num) # overlap patches:n
    # print(upsample_indices_1.shape, downsample_indices_2.shape)


    # initialize the target of matching matrix
    # bce_labelsl2s = torch.zeros(196, 196)
    # bce_labelss2l = torch.zeros(196, 196)
    bce_labelsl2s = torch.zeros(patch_num*patch_num, patch_num*patch_num*4)
    bce_labelss2l = torch.zeros(patch_num*patch_num, (patch_num//2)*(patch_num//2))

    # get 5*5 gaussian kernel
    if patch_num > 16:
        kernal1 = gaussian_kernel_normalized(size=7)
        kernal2 = gaussian_kernel_normalized(size=7)
    elif patch_num > 14:
        kernal1 = gaussian_kernel_normalized(size=7)
        kernal2 = gaussian_kernel_normalized(size=5)
    else:
    ...
    for i in range(len(true_indices_2)): 
        ...
        row, col = divmod(idx1.item(), patch_num*2)

        coords, weights = apply_gaussian_kernel(kernal1, (row, col), matrix_size=patch_num*2) # apply 5*5 gaussion weights to the matrx matching target

        for i in range(len(coords)):
            corresponding_idx = coords[i][0] * patch_num*2 + coords[i][1]
            # print(idx2, corresponding_idx)
    ...
    for i in range(len(true_indices_1)):
        ...
        row, col = divmod(idx2.item(), patch_num//2)

        coords, weights = apply_gaussian_kernel(kernal2, (row, col), matrix_size=patch_num//2) # apply 5*5 gaussion weights to the matrx matching target
...
# def get_patch_matching_target_1to1(idx1_pair, idx2_pair):
#     """
#     For crop1 and crop2 (both 14x14), generate a soft one-to-one matching matrix [196, 196],
#     where each patch in crop2 corresponds to a soft region in crop1 using a 5x5 Gaussian kernel.
#     """
#     crop_size = 14
#     gaussian = gaussian_kernel_normalized(5)  # [5, 5]
#     half_k = 2

#     target_matrix = torch.zeros(crop_size**2, crop_size**2)

#     for i in range(crop_size):
#         for j in range(crop_size):
#             # Absolute position in full grid
#             gx2, gy2 = idx2_pair[0] + i, idx2_pair[1] + j

#             # Center of kernel in crop1 coordinate
#             gx1_center = gx2 - idx1_pair[0]
#             gy1_center = gy2 - idx1_pair[1]

#             idx_crop2 = i * crop_size + j
...
#                     if 0 <= ni < crop_size and 0 <= nj < crop_size:
#                         idx_crop1 = ni * crop_size + nj
#                         target_matrix[idx_crop2, idx_crop1] = gaussian[dx + half_k, dy + half_k]
#                         print(f"idx_crop2: {idx_crop2}, idx_crop1: {idx_crop1}, value: {gaussian[dx + half_k, dy + half_k]}")
...
def get_patch_matching_target_1to1(overlap_mask_1, overlap_mask_2, patch_num=14):
    ...
    # get 5*5 gaussian kernel
    kernel = gaussian_kernel_normalized(size=5)
...
class PatchCrop():
    ...
    def __init__(self, size):
        self.size = size
    def __call__(self, image, k=1, l=1):
...
class PatchCropViT():
    ...
    def __call__(self, image, k=1, l=1):
        image = cv2.resize(image, (self.size, self.size), interpolation=cv2.INTER_AREA)
        grid = int(self.size/90)
        # k, l = choices([(1,1), (1,2), (2,1), (2,2)], [3, 1, 1, 1])[0]

        if k==2 and l==2:
            x1 = randint(0,16)
            y1 = randint(0,16)
            x2 = randrange(x1%2, 54, 2) # randrang不包括尾部的数
            y2 = randrange(y1%2, 54, 2)
        elif k==1 and l==1:
            x1 = randint(0,53)
            y1 = randint(0,53)
            x2 = randrange(max(x1-18,0), min(53, x1+18), 1)
            y2 = randrange(max(y1-18,0), min(53, y1+18), 1)
        

        patch1 = image[x1*grid:(37*l+x1)*grid, y1*grid:(37*k+y1)*grid, :]
        patch1 = cv2.resize(patch1, (518, 518))
        patch2 = image[x2*grid:(37+x2)*grid, y2*grid:(37+y2)*grid, :]
        image = np.concatenate((patch1, patch2), axis=2)
        return image, (x1, y1), (x2, y2), (k, l)


class PatchCropSwinV2():
    def __init__(self, size):
        self.size = size
    def __call__(self, image, k=1, l=1):
        grid = int(self.size/36) # image size 1152(36*32)

        if k==2 and l==2:
            x1 = randint(0,4)
            y1 = randint(0,4)
            x2 = randrange(x1%2, 21, 2) # randrang不包括尾部的数
            y2 = randrange(y1%2, 21, 2)
        elif k==1 and l==1:
            x1 = randint(0,18)
            y1 = randint(0,18)
        ...
        # ipdb.set_trace()
        patch1 = image[x1*grid:(16*l+x1)*grid, y1*grid:(16*k+y1)*grid, :]
        patch1 = cv2.resize(patch1, (512, 512))
        patch2 = image[x2*grid:(16+x2)*grid, y2*grid:(16+y2)*grid, :]
        image = np.concatenate((patch1, patch2), axis=2)
