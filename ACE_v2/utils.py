...
def load_pretrained_weights_student(model, pretrained_weights, checkpoint_key, model_name, patch_size):
    if os.path.isfile(pretrained_weights):
        state_dict = torch.load(pretrained_weights, map_location="cpu")
        if checkpoint_key is not None and checkpoint_key in state_dict:
            print(f"Take key {checkpoint_key} in provided checkpoint dict")
...
def init_from_imagenet(ckp_path, student, teacher):
    """
...
def init_from_imagenet_swinv2(ckp_path, student, teacher):
    """
    Re-start from checkpoint
    """
    if not os.path.isfile(ckp_path):
    ...
    # Problem of mismatching keys
    # keys = list(state_dict.keys())
    # with open('pretrained_model_keys.txt', 'w') as f:
    #     for key in keys:
    #         f.write(f"{key}\n")
    
    # keys_ = list(student.state_dict().keys())
    # with open('student_model_keys.txt', 'w') as f:
    #     for key in keys_:
    #         f.write(f"{key}\n")
    state_dict = {"module.backbone."+k: v for k, v in state_dict.items()}
    msg = student.load_state_dict(state_dict, strict=False)
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, log_writer=None):
        i = 0
        if not header:
            header = ''
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                if torch.cuda.is_available():
                    print(log_msg.format(
                        ...
                        memory=torch.cuda.max_memory_allocated() / MB), file=log_writer)
                else:
                    print(log_msg.format(
                        i, len(iterable), eta=eta_string,
                        meters=str(self),
                        time=str(iter_time), data=str(data_time)), file=log_writer)
                log_writer.flush()
            i += 1
            end = time.time()
...
class UpsampleLayer(nn.Module):
    def __init__(self, embed_dim=1024):
        ...
        self.conv_transpose = nn.ConvTranspose2d(in_channels=embed_dim, out_channels=embed_dim, kernel_size=2, stride=2)
    
    def forward(self, x, patch_num=14):
        x = rearrange(x,'b (m1 m2) c -> b c m1 m2', m1=patch_num, m2=patch_num) # 196*1024 --> 1024*14*14
        x = self.conv_transpose(x)
        x = rearrange(x,'b c m1 m2 -> b (m1 m2) c') # 1024*28*28 --> 1024*784
        return x
...
class DownsampleLayerMLP(nn.Module):
    ...
    def forward(self, x, patch_num=14): # x:[B,196,1024]
        x = rearrange(x,'b (m1 m2) c -> b c m1 m2',m1=patch_num,m2=patch_num) # 196*1024 --> 1024*14*14
        # x = x.reshape(B,1024,14,14) 
...
class MultiCropWrapper(nn.Module):
    ...
    def __init__(self, backbone, head, DenseHead, args):
        ...
        if 'swin' in self.args.arch:
            ...
            embding_dim = 768
        self.predictor_ = nn.Sequential(nn.Linear(embding_dim, 512, bias=False),
                                nn.LayerNorm(512),
                                nn.ReLU(inplace=True), # hidden layer
                                nn.Linear(512, embding_dim)) # output layer
        if self.args.arch == 'swin_base':
            self.global_head = GlobalFeatureMLP(num_tokens=196)
        elif self.args.arch == 'swinv2_base':
            self.global_head = GlobalFeatureMLP(num_tokens=256)
        self.downsample = DownsampleLayerMLP(embding_dim)
        self.upsample = UpsampleLayerMLP(embding_dim)
        ...
        if epoch%3==0:
            ...
            for end_idx in idx_crops:
                ...
                if 'swin' in self.args.arch:
                    ...
                    if isinstance(_out, tuple):
                        ...
                        out_spatial = _out[1]
                        # ipdb.set_trace()
                        _out = self.global_head(out_spatial)
                        # print(_out.shape)
                else:
                    _out = self.backbone(torch.cat(x[start_idx: end_idx]))
                    _out = _out.last_hidden_state[:,0] # cls token: [B,768]
            ...
            for end_idx in idx_crops: # idx_crops=[2]
                ...
                if 'swin' in self.args.arch:
                    ...
                    _out = self.backbone(torch.cat(x[start_idx: end_idx]))
                    _middle_features = _out.last_hidden_state[:,1:] # features: [B,37^2,768]
            ...
            return self.predictor_(spatial_features)
        
        elif epoch%3==2: # composition decomposition
            for end_idx in idx_crops: # idx_crops=[2]
                image_origin_input=torch.cat(x[start_idx: end_idx]) # concat in batchsize, [crop1,crop2]
                # if end_idx==2: # will go to this branch
                ...
                if self.args.arch == 'swin_base':
                    ...
                    spatial_features = self.DenseHead(_middle_features) # B*196*1024
                    upsample_features = self.upsample(spatial_features)
                    downsample_features = self.downsample(spatial_features, patch_num=16)
                elif self.args.arch == 'vit_base':
                    _out = self.backbone(torch.cat(x[start_idx: end_idx]))
                    _middle_features = _out.last_hidden_state[:,1:] # features: [B,37^2,768]
                    spatial_features = self.DenseHead(_middle_features) # B*196*1024
                    upsample_features = self.upsample(spatial_features)
                    downsample_features = self.downsample(spatial_features, patch_num=37)
            ...
            return upsample_features,downsample_features# global embedding, local embeddings of student
