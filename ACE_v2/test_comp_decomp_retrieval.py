...
def get_args_parser():
    ...
    parser.add_argument('--img_size', default=518, type=int, help="""Size in pixels of theinput image..""")
    # Misc
...
class ChestX_ray14_comp(Dataset):
    def __init__(self, pathImageDirectory, pathDatasetFile, patch_size=448, num_class=14):
        self.img_list = []
        self.img_label = []
...
def train_dino(args):
    ...
    dataset = ChestX_ray14_comp(args.data_path,'./data/xray14/official/test_official.txt', args.img_size)
    ...
    model = AutoModel.from_pretrained('/mnt/sda/zhouziyu/ssl/pretrained_model/huggingface/rad-dino',output_hidden_states=True) # dino-v2
    ...
    checkpoint = torch.load('/mnt/nvme1n1/zhouziyu/ACE_journal/ACE_v2_vit/pretrained_weight/vitb_fromDINOv2_3unique_multiscale_consis_compdecomp/checkpoint0075.pth', map_location='cpu')
...
def train_one_epoch(model,  data_loader):
    ...
    with torch.no_grad():
        for it, (origin_image, masked_image, patch, imageLabel) in enumerate(data_loader):
            ...
            spatial_features_origin_image = model(origin_image).last_hidden_state[:,0] # dinov2
            spatial_features_patch = model(patch).last_hidden_state[:,0]
            spatial_features_masked_image = model(masked_image).last_hidden_state[:,0]
