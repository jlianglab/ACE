...
class SwinTransformerSSL(nn.Module):
    r""" Swin Transformer
...
class MaskedAutoencoderViT(nn.Module):
    ...
    """
    def __init__(self, img_size=448, patch_size=32, in_chans=3,
                 decoder_embed_dim=1024, decoder_depth=8, decoder_num_heads=16,num_patches=196,
    ...
    def random_masking(self, x, mask_ratio):
        ...
        ids_restore = torch.argsort(ids_shuffle, dim=1) # 原位置索引

        # keep the first subset
        ids_keep = ids_shuffle[:, :len_keep]
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D)) # torch.gather 从原tensor中获取指定index的数据
...
class SwinTransformerForSimMIM(SwinTransformerSSL):
    """ Vision Transformer """
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        assert self.num_classes == 0
        self.mask_token = nn.Parameter(torch.zeros(1, 1, 1024)) # mae manner
        # self.mask_token = nn.Parameter(torch.zeros(1, 1, 128)) # simmim manner
        self._trunc_normal_(self.mask_token, std=.02)

    def _trunc_normal_(self, tensor, mean=0., std=1.):
        trunc_normal_(tensor, mean=mean, std=std, a=-std, b=std)

    def decoder_preprocess(self,mask_token, mask, x):
        """
        mask_token: [1,1,embd_dim]
        """
        mask_downsample = mask[:,::8,::8].type_as(x) # mask_downsample [B,14,14]
        B,H,W = mask_downsample.shape
        B,L,D = x.shape
        L1 = int(L ** 0.5)
        # x_reshape = x.deepcopy()
        x_reshape = x.reshape(B,L1,L1,D)
        cat_x = torch.zeros((B,H,W,D)).type_as(x)
        
        cat_x[mask_downsample==1]=x.reshape(B*L,D)
        cat_x = cat_x.view(B,H,W,D)
        mask_tokens = mask_token.expand(B, H*W, -1)
        # print(mask_token.shape)
        # ipdb.set_trace()
        mask_tokens = mask_tokens.reshape(B, H, W, D)
        # ipdb.set_trace()
        cat_x = cat_x+mask_tokens*(1. - mask_downsample.unsqueeze(-1).expand(-1,-1,-1,D)) # cat_x [B,14,14,D]
        cat_x = cat_x.contiguous().view(B,H*W,D) # cat_x [B,196,D]

        # # average x as cls_token for vit decoder
        # x_mean = x.mean(dim=1, keepdim=True)
        # cat_x = torch.cat((x_mean, cat_x), dim=1) # cat_x [B,197,D]
    ...
    def forward(self, x, mask=None, perm=None):
        ...
        B,L,D = x.shape
        if mask is not None: # mask shape: [B,112,112]
            # w = mask.flatten(1).unsqueeze(-1).repeat(1, 1, D) # [B,112^2,D]
            w = mask.flatten(1).unsqueeze(-1).expand_as(x) # [B,112^2]
            # print(w.shape)
            x = x[w==1]
            x = x.view(B,-1,D) # [B,(11*8)^2,D]
            # print(x.shape)
        ...
        # Process the embeddings before decoder
        if mask is not None: # concate the mask_tokens to the decoder
            cat_x = self.decoder_preprocess(self.mask_token, mask, x)
            return x.mean(dim=1), cat_x        
        else:
