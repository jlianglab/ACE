...
def update_config(config, args):
    ...
    if config.DATA.IMG_SIZE == 448:
        if config.LINEAR_PROB:
            config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+'_'+config.BACKBONE+'_linearprob_448_'+config.DATA.FOLD)
        else:
            config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+'_'+config.BACKBONE+'_448_'+config.DATA.FOLD)
    else:
        config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+'_'+config.BACKBONE+'_224_'+config.DATA.FOLD)
