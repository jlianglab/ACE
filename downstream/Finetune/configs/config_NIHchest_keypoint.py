...
_C.OUTPUT = '/sda/zhouziyu/ssl/downstream_checkpoints/NIHchest_keypoint_detect/'
...
_C.DATA.DATA_PATH = '/mnt/sda/zhouziyu/ssl/datasets/ChestXray/NIHChestX-ray14/images'
# dataset name
_C.DATA.DATASET = 'NIHchest'
# dataset lists and labels
_C.DATA.TRAIN_LIST = '/sda/zhouziyu/ssl/datasets/ChestXray/NIHChestX-ray14/Landmark_split/train'
_C.DATA.VAL_LIST = '/sda/zhouziyu/ssl/datasets/ChestXray/NIHChestX-ray14/Landmark_split/val'
_C.DATA.TEST_LIST = '/sda/zhouziyu/ssl/datasets/ChestXray/NIHChestX-ray14/Landmark_split/test'
...
def update_config(config, args):
    ...
    if _check_args('select_index'):
        config.SELECT_INDEX = args.select_index
    ...
    if _check_args('model_type'):
        config.MODEL.TYPE = args.model_type

    # output folder
    if config.DATA.IMG_SIZE == 448:
        if config.LINEAR_PROB:
            config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+config.BACKBONE+'_linearprob_448_'+config.DATA.FOLD)
        else:
            config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+config.BACKBONE+'_448_'+config.DATA.FOLD)
    elif config.DATA.IMG_SIZE == 224:
        config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+config.BACKBONE+'_224_'+config.DATA.FOLD)
    elif config.DATA.IMG_SIZE == 512:
        if config.LINEAR_PROB:
            config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+config.BACKBONE+'_linearprob_512_'+config.DATA.FOLD)
        else:
            config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+config.BACKBONE+'_512_'+config.DATA.FOLD)
    elif config.DATA.IMG_SIZE == 518:
        if config.LINEAR_PROB:
            config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+config.BACKBONE+'_linearprob_518_'+config.DATA.FOLD)
        else:
            config.OUTPUT = os.path.join(config.OUTPUT, config.PRETRAIN_MODE+config.BACKBONE+'_518_'+config.DATA.FOLD)
...
def get_config_NIHchest_keypoint(args):
