...
def build_loader_Montgomery(config, ddp):
    ...
    if config.MODE == 'train':
        ...
        train_dataset = Montgomery_dataset(image_root=image_root, mask_root=mask_root, datalist=train_list, img_transforms=img_train_transforms, config=config)
        
        img_val_transforms = img_transforms(mode='val', config=config)
        val_dataset = Montgomery_dataset(image_root=image_root, mask_root=mask_root, datalist=val_list, img_transforms=img_val_transforms, config=config)
        ...
        test_dataset = Montgomery_dataset(image_root=image_root, mask_root=mask_root, datalist=test_list, img_transforms=img_test_transforms, config=config)
        test_loader = DataLoader(dataset=test_dataset, 
                                batch_size=config.DATA.BATCH_SIZE, 
                                num_workers=config.DATA.NUM_WORKERS)
        return test_dataset, test_loader

class Montgomery_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask = cv2.imread(os.path.join(self.mask_root, self.datalist[index]))
        mask[mask==255]=1
