# SIIM dataset for classification
...
def build_loader_SIIM_cls(config, ddp):
    ...
    if config.MODE == 'train':
        ...
        if config.RATIO == '50':
            ...
            train_list = train_list[:int(train_num/20)]
        elif config.RATIO == '1':
...
class SIIM_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # mask[mask==255]=1
        label = torch.FloatTensor([int(np.any(mask) != 0 )])# disease label 0/1
