...
if __name__ == "__main__":
    ...
    parser.add_argument('--error_file', type=str, default='./correspondence/error_ACEv2_swinv2_symmetry_Lclavicle.csv',  help='save the errors of each predicted landmark.')
    ...
    for query_img in querydict.keys(): # 100 query images
        ...
        # elif args.pretrain_mode in ['CheSS']:
        #     shift = 28
        # patch = crop_and_pad(query_image, selected_coord, (args.crop_size,args.crop_size), shift)
        # patch = cv2.resize(patch, (args.img_size, args.img_size), interpolation=cv2.INTER_CUBIC)
        # patch = cv2.cvtColor(patch, cv2.COLOR_BGR2RGB)
        ...
        for key in keydict.keys(): # 100 key images
            ...
            for i in range(w_embd):
                for j in range(h_embd):
                    similarities[i, j] = cosine_similarity(E_syn, key_features[i, j].reshape(1, -1)).item()
            ...
            for i in range(w_embd):
                for j in range(h_embd):
                    similarities[i, j] = cosine_similarity(E_left_clavicle, key_features[i, j].reshape(1, -1)).item()
