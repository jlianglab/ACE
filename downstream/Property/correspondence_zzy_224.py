...
def save_embeddings(args, model, image, device, stride, embed_dim=1024):
    ...
    for i in range(w_embd):
        for j in range(h_embd):
            ...
            patch = crop_and_pad(image, center, (args.img_size,args.img_size), 32) # crop size of 448*448 image around the center
            ...
            with torch.no_grad():
                ...
                f1 = features[:,24] # 90 for swin backbone and 91 for vit backbone, 24 for 224 image size
...
if __name__ == "__main__":
    ...
    parser.add_argument('--embd_dir', type=str, default='/mnt/sda/zhouziyu/ssl/datasets/ChestXray/NIHChestX-ray14/landmark_embd_Adam_224_5imgs',  help='key image embeddings saving dictionary.')
    ...
    parser.add_argument('--error_file', type=str, default='./correspondence/error_Adam_224.csv',  help='save the errors of each predicted landmark.')
    ...
    parser.add_argument('--img_size', type=int, default=224,  help='save the key embeddings of the whole image')
        ...
        for j in key_imglist:
            ...
            with open(os.path.join(args.text_files_dir, labeldir[imglist[t]]), 'r') as f: # query image
                ...
                for pos in selected_positions:
                    ...
                    patch = crop_and_pad(query_image, selected_coord, (args.img_size,args.img_size), 32)
                    patch = cv2.resize(patch, (args.img_size, args.img_size), interpolation=cv2.INTER_CUBIC)
                    patch = cv2.cvtColor(patch, cv2.COLOR_BGR2RGB)
                    ...
                    with torch.no_grad():
                        ...
                        features = model.extract_features(patch) # convnext
                        # print(features.shape)
                        f1 = features[:,24] # 90 for swin backbone and 91 for vit backbone
