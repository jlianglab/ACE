...
def find_correspondences(image_path1: str, image_path2: str, num_pairs: int = 10, load_size: int = 224, layer: int = 9,
                         facet: str = 'key', bin: bool = True, thresh: float = 0.05, model_type: str = 'dino_vitb16',
                         stride: int = 4, indices_show:numpy.ndarray = None) -> Tuple[List[Tuple[float, float]], List[Tuple[float, float]],
    ...
    print('features shape:', descriptors1.shape)
    ...
    # keep indices the same

    # import pdb
    # pdb.set_trace()
    indices_to_show = [279,249,2233,2536,132, 1209,1281]
    # indices_to_show = torch.nonzero(bbs_mask, as_tuple=False).squeeze(dim=1)[
    #     bb_indices_to_show]  # close bbs
    # # import pdb
    # # pdb.set_trace()
    # if indices_show is not None:
    #     indices_to_show = indices_show
    ...
    return points1, points2, image1_pil, image2_pil, indices_to_show
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root_dir', type=str, default='/sda1/zhouziyu/ssl/dataset/NIHChestX-ray14/images/',  help='The root dir of image pairs.')
    parser.add_argument('--save_dir', type=str, default='./corres_sample15/', help='The root save dir for image pairs results.')
    parser.add_argument('--load_size', default=448, type=int, help='load size of the input image.')
    ...
    with torch.no_grad():
        ...
        #curr_images = [args.root_dir+'/00006621_001.png',args.root_dir+'/00006621_002.png',args.root_dir+'/00006642_011.png',args.root_dir+'/transformed_image_2.jpg',args.root_dir+'/00006641_001.png']
        curr_images = [args.root_dir + '/00007551_017.png', args.root_dir + '/00007207_001.png',
                       args.root_dir + '/00007444_003.png', args.root_dir + '/00007569_017.png',
                       args.root_dir + '/transformed_image_2.jpg']
        #assert len(curr_images) == 2, f"{pair_dir} contains {len(curr_images)} images instead of 2."
        curr_save_dir = save_dir / '0'
        curr_save_dir.mkdir(parents=True, exist_ok=True)

        # compute point correspondences
        points1, points2, image1_pil, image2_pil, bb_indices_to_show = find_correspondences(curr_images[0], curr_images[1],
                                                                        args.num_pairs, args.load_size, args.layer,
                                                                        args.facet, args.bin, args.thresh)
        print(type(bb_indices_to_show))
        ...
        for i in range(3):
            print(i)
            points1, points2, image1_pil, image2_pil, _ = find_correspondences(curr_images[0],
                                                                                curr_images[i+2],
                                                                                args.num_pairs,
                                                                                args.load_size,
                                                                                args.layer,
                                                                                ...
                                                                                args.bin,
                                                                                args.thresh,
                                                                                indices_show = bb_indices_to_show)#
# saving point correspondences
#             file1 = open(curr_save_dir / "correspondence_A"+str(i)+".txt", "w")
#             file2 = open(curr_save_dir / "correspondence_Bt"+str(i)+".txt", "w")
#             for point1, point2 in zip(points1, points2):
#                 file1.write(f'{point1}\n')
#                 file2.write(f'{point2}\n')
#             file1.close()
#             file2.close()
            ...
            fig1, fig2 = draw_correspondences(points1, points2, image1_pil, image2_pil)
            fig1.savefig(curr_save_dir / f'{Path(curr_images[0]).stem}_23corresp.png', bbox_inches='tight',
                          pad_inches=0)
            fig2.savefig(curr_save_dir / f'{Path(curr_images[i+2]).stem}_23corresp.png', bbox_inches='tight',
                         pad_inches=0)
            plt.close('all')