...
if __name__ == "__main__":
    ...
    parser.add_argument('--query_list', type=str, default='./correspondence/query_key_list/landmark_key_list.txt',  help='we have not save the embedding dict of query images, so we use key images')
    ...
    parser.add_argument('--error_file', type=str, default='./correspondence/error_ACEv2_swinv2_symmetry_Lclavicle_intra_patient.csv',  help='save the errors of each predicted landmark.')
    ...
    landmark_name = ['QueryImage', 'Syn Error intra-patient', 'GT Error intra-patient']
    ...
    for query_img in querydict.keys(): # 100 query images
        ...
        # selected_coord_P = coords_query[selected_positions[0]] # left clavicle
        
        # gt_embd_P, predict_embed_P, similarity = interpolation(selected_coord_A, selected_coord_P, query_image, model, device, args)
        # print(similarity)
        
        
        right_clavicle = coords_query[selected_positions[1]] # right clavicle
        left_rib4 = coords_query[selected_positions[4]] # left rib4
        right_rib4 = coords_query[selected_positions[5]] # right rib4
        left_clavicle = coords_query[selected_positions[0]] # left clavicle
        
        E_right_clavicle = get_embd(model, right_clavicle, query_image, device, args).cpu().numpy()
        E_left_rib4 = get_embd(model, left_rib4, query_image, device, args).cpu().numpy()
        E_right_rib4 = get_embd(model, right_rib4, query_image, device, args).cpu().numpy() 
        E_left_clavicle = get_embd(model, left_clavicle, query_image, device, args).cpu().numpy()
        
        E_syn = E_right_clavicle + E_left_rib4 - E_right_rib4
        similarity = cosine_similarity(E_left_clavicle.reshape(1, -1), E_syn.reshape(1, -1))[0][0]
        print(similarity)
        
        # compute the error of predict embedding P retrieve the landmark position intra-patient
        query_pred_position = [] # save the most similar embedding's positions | predict embeding P with the embedding of query image
        query_features = np.load(os.path.join(args.embd_dir, query_img+'.npy'))
        similarities = np.zeros((w_embd, h_embd))
        for i in range(w_embd):
            for j in range(h_embd):
                similarities[i, j] = cosine_similarity(E_syn, query_features[i, j].reshape(1, -1)).item()
        ...
        print((x_pred, y_pred), left_clavicle)
        # query_pred_position.append((x_pred, y_pred))
        error = math.sqrt((x_pred-left_clavicle[0])**2+(y_pred-left_clavicle[1])**2)
        error_row.append(error) # error of predict embedding P intra-patient
        
        
        similarities2 = np.zeros((w_embd, h_embd))
        for i in range(w_embd):
            for j in range(h_embd):
                similarities2[i, j] = cosine_similarity(E_left_clavicle, query_features[i, j].reshape(1, -1)).item()

        max_index = np.unravel_index(np.argmax(similarities2, axis=None), similarities2.shape)
        x_pred, y_pred = int(max_index[0]*args.stride+args.stride//2), int(max_index[1]*args.stride+args.stride//2)
        print((x_pred, y_pred), left_clavicle)
        # query_pred_position.append((x_pred, y_pred))
        error = math.sqrt((x_pred-left_clavicle[0])**2+(y_pred-left_clavicle[1])**2)
        ...
        #     # else:
        #     #     continue
        #     key_gt_position.append(coords_key[selected_positions[0]]) # left clavicle
        ...
        #     max_index = np.unravel_index(np.argmax(similarities, axis=None), similarities.shape)
        #     x_pred, y_pred = int(max_index[0]*args.stride+args.stride//2), int(max_index[1]*args.stride+args.stride//2)
        #     print((x_pred, y_pred), coords_key[selected_positions[0]])
        ...
        # error_100key = error_100key/100
        # error_row.append(error_100key)
        # error_row.append(similarity) # similarity of predict embeding P and gt embeding P
