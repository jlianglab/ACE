# 模型路径在ViTExtractor中修改，在n_cluster_range = list(range(1, 9))中修改类别数
...
def find_cosegmentation(image_paths: List[str], elbow: float = 0.975, load_size: int = 224, layer: int = 11,
    ...
    n_cluster_range = list(range(1, 9)) # 类别数
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root_dir', type=str, default='co-segmentation/root_image', help='The root dir of image sets.')
    parser.add_argument('--save_dir', type=str, default='co-segmentation/vitb_fromdinov2_518', help='The root save dir for image sets results.')
    parser.add_argument('--load_size', default=518, type=int, help='load size of the input images. If None maintains'
                                                                    'original image size, if int resizes each image'
                                                                    'such that the smaller side is this number.')
    parser.add_argument('--stride', default=4, type=int, help="""stride of first convolution layer. 
                                                                 small stride -> higher resolution.""")
    parser.add_argument('--model_type', default='dinov2', type=str,
    ...
    #                                                                      "clustering.")
    parser.add_argument('--sample_interval', default=20, type=int, help="sample every ith descriptor for training"
                                                                         "clustering.")
