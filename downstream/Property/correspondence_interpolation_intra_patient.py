...
if __name__ == "__main__":
    ...
    parser.add_argument('--error_file', type=str, default='./correspondence/error_ACEv2_swinv2_interpolation_Lclavicle_intra_patient.csv',  help='save the errors of each predicted landmark.')
    ...
    landmark_name = ['QueryImage', 'Error intra-patient']
    ...
    for query_img in querydict.keys(): # 100 query images
        ...
        for i in range(w_embd):
            for j in range(h_embd):
                similarities[i, j] = cosine_similarity(predict_embed_P, query_features[i, j].reshape(1, -1)).item()
        ...
        print((x_pred, y_pred), selected_coord_P)
        # query_pred_position.append((x_pred, y_pred))
        error = math.sqrt((x_pred-selected_coord_P[0])**2+(y_pred-selected_coord_P[1])**2)
