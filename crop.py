...
def build_loader_compose(config):
    ...
    train_dataset = NIHchest_dataset(
...
def get_index(a, b, c):
    ...
    input:
      ...
      c: the h, w rate of crop1, (16*k)*(16*l)
    output: the overlap mask of crop1 and crop2, and the shape of overlap masks are all 16*16
    """
    (idx_x1, idx_y1), (idx_x2, idx_y2), (k, l) = a, b, c

    # 重合部分index范围
    idx_xmin, idx_xmax = max(idx_x1, idx_x2), min((idx_x1 + 16 * l), (idx_x2 + 16))
    idx_ymin, idx_ymax = max(idx_y1, idx_y2), min((idx_y1 + 16 * k), (idx_y2 + 16))

    # 找出重合部分在crop1中对应的index list
    overlap_mask_1 = torch.zeros((16, 16))
    overlap_mask_1[
        (idx_ymin - idx_y1) // k : (idx_ymax - idx_y1) // k,
        (idx_xmin - idx_x1) // l : (idx_xmax - idx_x1) // l,
    ] = 1
    overlap_mask_1 = overlap_mask_1.flatten()
    # index1 = torch.nonzero(overlap_mask_1)
    # print(index1)

    overlap_mask_2 = torch.zeros((16, 16))
    overlap_mask_2[
        idx_ymin - idx_y2 : idx_ymax - idx_y2, idx_xmin - idx_x2 : idx_xmax - idx_x2
    ] = 1
...
def get_corresponding_indices(overlap_mask_1, overlap_mask_2, idx1_pair, idx2_pair, kl):
    """
    input:
     overlap_mask_1: overlap index of crop1 (bool, 16*16)
     overlap_mask_2: overlap index of crop2 (bool, 16*16)
     idx1_pair: top left grid index of crop1
     idx2_pair: top left grid index of crop2
     kl: the size rate of crop1
    output: two target matrices of matrix matching, size 256*256
    ...
    mapping_array_2_to_1 = np.full(
        overlap_mask_2.numel(), -1, dtype=int
    )  # shape: (256,)
    ...
    bce_labelsl2s = torch.zeros(256, 256)
    bce_labelss2l = torch.zeros(256, 256)

    if true_indices_2.dim() != 0:
        # 如果是0维张量（标量），则跳过循环
        for idx in true_indices_2:
            # 计算在 16x16 矩阵中的行和列
            row, col = divmod(idx.item(), 16)

            # 计算对应的位置在 overlap_mask_1 中的索引
            corresponding_row = (row + offset_y) // k
            corresponding_col = (col + offset_x) // l
            corresponding_idx = corresponding_row * 16 + corresponding_col

            # 更新映射数组
            mapping_array_2_to_1[idx] = (
                corresponding_idx if overlap_mask_1[corresponding_idx] else -1
            )
...
class GridRandomCrop:  # 608*608 -> 576*576 / 304*304 -> 288*288
    ...
    def __call__(self, image):
        grid_size = int(
            self.size / 17
        )  # the image is cropped into 19*19 grids, and we take 18*18 grids from it
        start_x = randint(0, grid_size - 1)
        start_y = randint(0, grid_size - 1)
        image = image[
            start_x : grid_size * 16 + start_x, start_y : grid_size * 16 + start_y, :
        ]
        return image
...
class PatchCrop:
    ...
    image: two concated crops which are resized to 448*448, the size of crop2 is fixed with 16*16 patches (imgsize 448*448), and the size of crop1 is (16*k)*(16*l)
    (x1, y1): the grid index of crop1's top left corner
    (x2, y2): the grid index of crop2's top left corner, the size of crop2 is fixed in 16*16 patches
    ...
    def __call__(self, image):
        ...
        if k == 2 and l == 2:
            x1 = randint(0, 4)
            y1 = randint(0, 4)
            x2 = randrange(x1 % 2, 17, 2)  # randrang不包括尾部的数
            y2 = randrange(y1 % 2, 17, 2)
        elif k == 2 and l == 1:  # 竖着的矩形
            x1 = randint(0, 16)
            y1 = randint(0, 4)
            x2 = randrange(max(x1 - 8, 0), min(19, x1 + 8), 1)
            y2 = randrange(y1 % 2, 17, 2)
        elif k == 1 and l == 2:  # 横着的矩形
            x1 = randint(0, 4)
            y1 = randint(0, 16)
            x2 = randrange(x1 % 2, 17, 2)
            y2 = randrange(max(y1 - 8, 0), min(17, y1 + 8), 1)
        elif k == 1 and l == 1:
            x1 = randint(0, 16)
            y1 = randint(0, 16)
            x2 = randrange(max(x1 - 8, 0), min(17, x1 + 8), 1)
            y2 = randrange(max(y1 - 8, 0), min(17, y1 + 8), 1)

        patch1 = image[
            x1 * grid : (16 * l + x1) * grid, y1 * grid : (16 * k + y1) * grid, :
        ]
        patch1 = cv2.resize(patch1, (512, 512))
        patch2 = image[x2 * grid : (16 + x2) * grid, y2 * grid : (16 + y2) * grid, :]
        patch2 = cv2.resize(patch2, (512, 512))
...
class PatchResize:
    ...
    def __call__(self, image, a, b, c):
        patch1 = image[:, :, 0:3]
        patch2 = image[:, :, 3:6]
        patch1 = cv2.resize(patch1, (512, 512), interpolation=cv2.INTER_AREA)
        patch2 = cv2.resize(patch2, (512, 512), interpolation=cv2.INTER_AREA)
...
def local_pixel_shuffling(x, prob=0.5):
    ...
    for _ in range(num_block):
        ...
        window = window.reshape((img_deps, block_noise_size_x, block_noise_size_y))
        image_temp[
            :,
            noise_x : noise_x + block_noise_size_x,
            noise_y : noise_y + block_noise_size_y,
...
def image_out_painting(x):
    ...
    while cnt > 0 and random.random() < 0.95:
        block_noise_size_x = img_rows - random.randint(
            3 * img_rows // 8, 4 * img_rows // 8
        )
        block_noise_size_y = img_cols - random.randint(
            3 * img_cols // 8, 4 * img_cols // 8
        )
        noise_x = random.randint(3, img_rows - block_noise_size_x - 3)
        noise_y = random.randint(3, img_cols - block_noise_size_y - 3)
        x[
